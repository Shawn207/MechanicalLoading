...
void user()
{	
	od_write(0x60FE, 0x01, 0x00);//clear DO3
	preset();
...
void preset()
{
	...
	lowerbytePeriod = (od_read(0x60FD,0x00)>>17)&0x0F;// read the DI 2-5 as upperbyte of period
	...
	upperbytePeriod = (od_read(0x60FD,0x00)>>17)&0x0F;// read the DI 2-5 as upperbyte of period
	od_write(0x60FE,0x01,0x20000);// write 1 through DO 2, telling Arduino that I have received upperbyte of period
	period = lowerbytePeriod + (upperbytePeriod<<4);
	//test
	od_write(0x60FF,0x00,period);
